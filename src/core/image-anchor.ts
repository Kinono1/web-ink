...
export function captureImage(image: HTMLImageElement): ImageTarget {
  ...
  const images = Array.from(doc.images).filter((candidate) => sameSource(candidate, candidates));
  return {
...
export function resolveImage(
  ...
  const sourceMatches = all.filter((image) => sameSource(image, target.sourceCandidates.length ? target.sourceCandidates : [target.src]));
  if (!sourceMatches.length) return { status: 'unresolved', reason: 'No image with the recorded source was found.' };
  const ready = sourceMatches.filter((image) => image.complete && image.naturalWidth > 0 && image.naturalHeight > 0);
  if (!ready.length) return { status: 'pending', reason: 'A matching image is present but has not loaded.' };

  const candidates = ready.filter((image) => compatibleDimensions(image, target)).filter((image) => {
    const selectorMatch = matchesSelector(image, target.selector);
    const altMatch = Boolean(target.alt) && image.alt.trim() === target.alt;
    const contextMatch = Boolean(target.context) && imageContext(image) === target.context;
  ...
  if (candidates.length !== 1) {
    return {
      status: 'unresolved',
      reason: candidates.length ? 'Matching images remain ambiguous.' : 'Source matched, but its selector, dimensions, alt text, or context did not verify.',
    };
  }
...
function sourceCandidates(image: HTMLImageElement): string[] {
  const values = [image.currentSrc, image.src, image.getAttribute('src'), image.getAttribute('data-src')]
...
function absoluteSource(value: string, doc: Document): string {
  try { return new URL(value, doc.baseURI).href; } catch { return value; }
}

function compatibleDimensions(image: HTMLImageElement, target: ImageTarget): boolean {
  ...
  return Math.abs(targetRatio - actualRatio) <= 0.002 * Math.max(1, targetRatio);
}

function matchesSelector(image: HTMLImageElement, selector: string): boolean {
  try { return Boolean(selector) && image.matches(selector); } catch { return false; }
}

function imageContext(image: HTMLImageElement): string {
  const figure = image.closest('figure');
  const caption = figure?.querySelector('figcaption');
  const nearby = caption ?? image.closest('p,li,article,main,section,div') ?? image.parentElement;
...
function selectorFor(image: HTMLImageElement): string {
  ...
  const parts: string[] = [];
  for (let current: Element | null = image; current && current.tagName !== 'HTML'; current = current.parentElement) {
