...
function invalid(message: string): never { throw new ValidationError(message); }
...
function exactKeys(value: Record<string, unknown>, name: string, required: string[], optional: string[] = []): void {
  const allowed = new Set([...required, ...optional]);
  for (const key of Object.keys(value)) if (!allowed.has(key)) invalid(`${name} has an unknown field: ${key}`);
  for (const key of required) if (!(key in value)) invalid(`${name}.${key} is required`);
}

function text(value: unknown, name: string, max: number, allowEmpty = true): string {
  if (typeof value !== 'string' || value.length > max || (!allowEmpty && value.length === 0) || /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(value)) {
...
function integer(value: unknown, name: string, min: number, max: number): number {
  if (!Number.isSafeInteger(value) || (value as number) < min || (value as number) > max) invalid(`${name} is out of range`);
...
function color(value: unknown, name: string): string {
  ...
  if (!/^#[0-9a-f]{6}$/.test(result)) invalid(`${name} must be a six-digit hex color`);
  return result;
}
...
function identifier(value: unknown, name: string): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(result)) invalid(`${name} has an invalid format`);
  return result;
}
...
function httpSource(value: unknown, name: string): string {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(url.protocol)) invalid(`${name} must be an HTTP(S) image URL`);
    return source;
  } catch {
...
function safeCurrentImageSource(value: unknown, pageUrl: string, name: string): string {
  const source = text(value, name, 8_192, false);
  if (source.startsWith('data:')) {
    if (!/^data:image\/(?:png|jpeg|gif|webp);base64,[A-Za-z0-9+/=\r\n]+$/i.test(source)) invalid(`${name} has an unsafe data image URL`);
    return source;
  }
  if (source.startsWith('blob:')) {
    try {
      ...
      if (blobUrl.origin !== page.origin) invalid(`${name} blob URL must belong to this page origin`);
      return source;
    } catch {
...
function validateTextTarget(value: unknown): TextTarget {
  ...
  exactKeys(target, 'annotation.target', ['exact', 'prefix', 'suffix', 'start', 'end', 'rootSelector'], ['containerId']);
  ...
  if (end - start !== exact.length) invalid('annotation.target exact text and offsets disagree');
  const containerId = target.containerId === undefined ? undefined : text(target.containerId, 'annotation.target.containerId', 512, false);
  return {
    exact,
    ...
    rootSelector: text(target.rootSelector, 'annotation.target.rootSelector', 2_048, false),
...
function validatePoint(value: unknown, index: number): Point {
  ...
  exactKeys(point, `annotation.shape.points[${index}]`, ['x', 'y'], ['pressure']);
  const pressure = point.pressure === undefined ? undefined : finite(point.pressure, `annotation.shape.points[${index}].pressure`, 0, 1);
  return {
...
function validateShape(value: unknown): ImageShape {
  ...
  if (shape.kind !== 'rectangle' && shape.kind !== 'ellipse' && shape.kind !== 'arrow' && shape.kind !== 'pen') invalid('annotation.shape.kind is unsupported');
  if (!Array.isArray(shape.points)) invalid('annotation.shape.points must be an array');
  const limit = shape.kind === 'pen' ? 2_000 : 2;
  const minimum = shape.kind === 'pen' ? 1 : 2;
  if (shape.points.length < minimum || shape.points.length > limit) invalid('annotation.shape.points count is invalid for its shape kind');
  return {
    kind: shape.kind,
...
function validateImageTarget(value: unknown, pageUrl: string): ImageTarget {
  const target = object(value, 'annotation.target');
  exactKeys(target, 'annotation.target', ['src', 'sourceCandidates', 'alt', 'naturalWidth', 'naturalHeight', 'selector', 'occurrence', 'context']);
  if (!Array.isArray(target.sourceCandidates) || target.sourceCandidates.length > 20) invalid('annotation.target.sourceCandidates must contain at most 20 sources');
  return {
    src: safeCurrentImageSource(target.src, pageUrl, 'annotation.target.src'),
    sourceCandidates: target.sourceCandidates.map((source, index) => safeCurrentImageSource(source, pageUrl, `annotation.target.sourceCandidates[${index}]`)),
    alt: text(target.alt, 'annotation.target.alt', 1_000),
    naturalWidth: integer(target.naturalWidth, 'annotation.target.naturalWidth', 1, 100_000),
    naturalHeight: integer(target.naturalHeight, 'annotation.target.naturalHeight', 1, 100_000),
    selector: text(target.selector, 'annotation.target.selector', 2_048, false),
    occurrence: integer(target.occurrence, 'annotation.target.occurrence', 0, 100_000),
...
export function validateAnnotation(value: unknown): Annotation {
  ...
  const base = {
    ...
    revision: integer(annotation.revision, 'annotation.revision', 0, MAX_REVISION),
  };
...
export function validateSettings(value: unknown): Settings {
  ...
  const disabledOrigins = settings.disabledOrigins.map((origin, index) => {
    const raw = text(origin, `settings.disabledOrigins[${index}]`, 2_048, false);
    try {
      const url = new URL(raw);
      if (!['http:', 'https:'].includes(url.protocol) || url.origin !== raw) invalid(`settings.disabledOrigins[${index}] must be an HTTP(S) origin`);
      return raw;
    } catch {
...
function jsonBytes(value: unknown): number {
  try { return new TextEncoder().encode(JSON.stringify(value)).byteLength; } catch { invalid('backup must be JSON-serializable'); }
}
...
export function validateBackup(value: unknown): BackupEnvelope {
  if (jsonBytes(value) > MAX_BACKUP_BYTES) invalid(`backup exceeds ${MAX_BACKUP_BYTES} bytes`);
...
export function validatedDefaultSettings(): Settings {
  return validateSettings({ language: 'zh-CN', defaultColor: COLORS[0], disabledOrigins: [] });
}
