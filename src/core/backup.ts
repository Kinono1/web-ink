import { SCHEMA_VERSION, type Annotation, type BackupEnvelope, type Settings } from './model';
...
export function serializeBackup(annotations: Annotation[], settings?: Settings): BackupEnvelope {
...
export function toMarkdown(annotations: Annotation[]): string {
  ...
  for (const annotation of [...annotations].sort((a, b) => a.pageUrl.localeCompare(b.pageUrl) || a.id.localeCompare(b.id))) {
    ...
    if (annotation.tags.length) lines.push(`- Tags: ${annotation.tags.map(markdownField).join(', ')}`);
