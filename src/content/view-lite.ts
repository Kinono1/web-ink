...
export function createView() {
  const host = document.createElement('web-ink-ui'); host.dataset.webInk = 'true'; host.dataset.webInkUi = 'true';
  ...
  const selection = document.createElement('div'); selection.className = 'bar selection';
  const drawing = document.createElement('div'); drawing.className = 'bar drawing';
  ...
  root.append(style, svg, selection, drawing, toast); document.documentElement.append(host);
  const applyPreferences = (settings: Settings) => { host.dataset.webInkTheme = settings.theme ?? 'system'; host.dataset.webInkReduceMotion = String(settings.reduceMotion === true); host.dataset.webInkReduceTransparency = String(settings.reduceTransparency === true); };
  applyPreferences({ language:'zh-CN', defaultColor:'#facc15', disabledOrigins:[], theme:'system', reduceMotion:false, reduceTransparency:false });
  return { host, root, svg, selection, drawing, toast, applyPreferences };
}
export function button(label: string, action: () => void, title?: string): HTMLButtonElement { const b = document.createElement('button'); b.type = 'button'; b.textContent = label; b.setAttribute('aria-label', title ?? label); b.title = title ?? label; b.addEventListener('click', action); return b; }
