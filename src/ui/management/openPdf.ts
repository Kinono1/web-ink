...
export async function openPdfTab(url: string): Promise<void> {
  ...
  const normalizeSource = (raw: string | null) => {
    if (!raw) return null;
    const source = new URL(raw); source.hash = ""; return source.href;
  };
  const requestedSource = normalizeSource(target.searchParams.get("source"));
  ...
  const existing = tabs.find((tab) => {
    ...
    return (!requestedSource && !requestedDocument) ||
      ((!requestedSource || normalizeSource(candidate.searchParams.get("source")) === requestedSource) && (!requestedDocument || candidate.searchParams.get("document") === requestedDocument));
  });
