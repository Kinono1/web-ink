...
export function StoragePanel({ language }: { language: Language }) {
  ...
  useEffect(() => {
    ...
    const listener = (message: { type?: string }) => {
      if (!['annotations.changed', 'settings.changed', 'page.mode.changed'].includes(message?.type ?? '')) return;
      clearTimeout(timer); timer = setTimeout(() => { void refresh(); }, 300);
    };
    chrome.runtime.onMessage.addListener(listener);
    return () => { active.current = false; clearTimeout(timer); chrome.runtime.onMessage.removeListener(listener); };
  }, [refresh]);
  const tooLarge = stats && (stats.backupBytes > stats.backupLimitBytes || stats.annotationCount > stats.backupRecordLimit);
  ...
  return <section className="storage-panel" aria-label={zh ? '本地存储' : 'Local storage'} aria-busy={loading}>
    ...
    {stats ? <>
      ...
      <details><summary>{zh ? '占用详情' : 'Usage details'}</summary>
        ...
        <p className="storage-muted">{zh ? '标注大小按记录的文本编码估算，实际数据库还包含索引等开销。浏览器估算不等于精确磁盘用量；以上均不含开发依赖和测试浏览器。' : 'Record size is estimated from its text encoding; database indexes add overhead. Browser estimates are not exact disk usage. Development tools and test browsers are excluded.'}</p>
      </details>
      {tooLarge ? <p className="storage-warning" role="status">{zh ? '已超过当前单份备份上限（20 MiB / 50,000 条），完整备份可能无法导出。请保留浏览器数据；分卷备份尚未提供。' : 'Above the current single-backup limit (20 MiB / 50,000 records). Full export may fail. Keep your browser data; split backups are not available yet.'}</p> : nearBackupLimit ? <p className="storage-warning" role="status">{zh ? '数据正在接近单份备份上限（20 MiB / 50,000 条），建议现在先导出一份备份。' : 'Approaching the single-backup limit (20 MiB / 50,000 records). Export a backup soon.'}</p> : null}
      {stats.logicalBytes >= stats.storageWarningBytes || quotaNear ? <p className="storage-warning" role="status">{zh ? '存储用量较高。长笔记和复杂笔迹会增加占用；此提醒不会删除或阻止保存标注。' : 'Storage usage is high. Long notes and detailed drawings increase usage; this notice does not delete or block annotations.'}</p> : null}
    </> : !error ? <p className="storage-muted">{zh ? '正在统计…' : 'Calculating…'}</p> : null}
  </section>;
}
