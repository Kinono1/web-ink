...
export class PageContextError extends Error {
  ...
  constructor() {
    super("The page changed. Please retry on the current page.");
  }
}
    ...
    const probe = (async () => {
      try {
        ...
        const result = results.find(
          (value) => value.frameId === 0 && value.documentId === documentId,
        );
        const locationResult = result?.result;
        if (
          !locationResult?.top ||
          !isWebPage(locationResult.href) ||
          new URL(locationResult.href).origin !== new URL(sender.url!).origin
        )
