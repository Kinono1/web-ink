...
const validEstimate = (value: unknown): number | null =>
  typeof value === "number" && Number.isFinite(value) && value >= 0
    ? value
    : null;

const emptyStats = (): LibraryStatsRecord => ({
  id: "library",
  annotationCount: 0,
  textCount: 0,
  imageCount: 0,
...
export async function applyIncrementalStats(change: {
  ...
  const stats = structuredClone(
    (await db.stats.get("library")) ?? emptyStats(),
  );
  if (change.beforeAnnotation)
    applyAnnotation(stats, change.beforeAnnotation, -1);
...
export async function recalculateStorageStats(): Promise<LibraryStatsRecord> {
  ...
  return db.transaction("rw", db.annotations, db.pages, db.stats, async () => {
    ...
    await db.annotations.each((annotation) =>
      applyAnnotation(stats, annotation, 1),
    );
...
export async function getStorageStats(
  ...
  const snapshot = recalculate
    ...
    : ((await db.stats.get("library")) ?? (await recalculateStorageStats()));
  let browserUsageBytes: number | null = null,
    browserQuotaBytes: number | null = null;
  try {
    const estimate = await globalThis.navigator?.storage?.estimate();
    browserUsageBytes = validEstimate(estimate?.usage);
    browserQuotaBytes = validEstimate(estimate?.quota);
  } catch {
    /* Some profiles do not expose origin estimates; unknown must not be shown as zero. */
  }
  const emptyBackupBytes = bytes({
    format: "web-ink",
  ...
  return {
    ...
    pdfTextCount: snapshot.pdfTextCount,
    pdfAreaCount: snapshot.pdfAreaCount,
    pageCount: snapshot.pageCount,
    logicalBytes:
      snapshot.annotationBytes + snapshot.pageBytes + bytes(settings),
    // Empty [] is already included; add records and separating commas. Sorting changes no byte count.
    backupBytes:
      emptyBackupBytes +
      snapshot.annotationBytes +
      Math.max(0, snapshot.annotationCount - 1),
    browserUsageBytes,
    browserQuotaBytes,
    backupLimitBytes: MAX_BACKUP_BYTES,
    backupRecordLimit: MAX_BACKUP_ANNOTATIONS,
