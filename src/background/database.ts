...
export class WebInkDatabase extends Dexie {
  ...
  constructor(name = "web-ink") {
    ...
    this.version(2)
      .stores({
        annotations:
          "id, pageUrl, updatedAt, [updatedAt+id], [pageUrl+updatedAt+id], [kind+updatedAt+id], [color+updatedAt+id], *tags",
        pages: "url, updatedAt",
        stats: "id",
      })
      .upgrade(async (transaction) => {
        let annotationCount = 0,
          ...
          pageCount = 0,
          pageBytes = 0;
        const annotations = transaction.table("annotations") as Table<
          Annotation,
          string
        >;
        ...
        await annotations.each((annotation) => {
          annotationCount += 1;
          annotationBytes += new TextEncoder().encode(
...
export function setDatabaseForTesting(
  database: WebInkDatabase | undefined,
): void {
