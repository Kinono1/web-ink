...
test('library editing, conflict preservation, JSON download and import work through the UI', async () => {
  ...
  const card = manager.locator('.record').first();
  await expect(card).toBeVisible();
  await card.getByRole('button', { name: '笔记', exact: true }).click();
  await card.getByRole('textbox', { name: '笔记', exact: true }).fill('我的草稿：保留到确认保存。');
  const [record] = await rpc<any[]>(manager, { type: 'annotations.list' });
  ...
  await expect(card.getByRole('textbox', { name: '笔记', exact: true })).toHaveValue('我的草稿：保留到确认保存。');
  await card.getByRole('button', { name: '载入最新版本', exact: true }).click();
  await card.getByRole('button', { name: '保存', exact: true }).click();
  await expect(card.locator('.note')).toHaveText('我的草稿：保留到确认保存。');
  await manager.getByText('备份与导入', { exact: true }).click();
  ...
  await expect(manager.locator('.record')).toHaveCount(0);
  ...
  await expect(manager.locator('.record')).toHaveCount(1);
