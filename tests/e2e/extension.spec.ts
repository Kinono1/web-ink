...
async function start(profilePath: string) {
  context = await chromium.launchPersistentContext(profilePath, {
    ...
    args: [
      "--headless=new",
      `--disable-extensions-except=${activeExtensionPath}`,
      `--load-extension=${activeExtensionPath}`,
    ],
...
test.beforeAll(async () => {
  ...
  grantedExtensionPath = await mkdtemp(
    path.join(tmpdir(), "web-ink-granted-build-"),
  );
  await cp(extensionPath, grantedExtensionPath, { recursive: true });
  const manifest = JSON.parse(
    await readFile(path.join(grantedExtensionPath, "manifest.json"), "utf8"),
  );
  manifest.host_permissions = origins;
  await writeFile(
    path.join(grantedExtensionPath, "manifest.json"),
    JSON.stringify(manifest),
  );
});
test.afterAll(async () => {
  if (grantedExtensionPath)
    await rm(grantedExtensionPath, { recursive: true, force: true });
});
test.beforeEach(async ({}, info) => {
  activeExtensionPath = info.title.startsWith("production permission")
    ? extensionPath
    : grantedExtensionPath;
...
async function article(activate = true) {
  ...
  if (activate && (await toggle.getAttribute("aria-pressed")) === "false")
    await toggle.click();
...
async function selectAndMark(page: Page, selector = "#selection strong") {
  ...
  await expect
    .poll(
      async () =>
        (await rpc<any[]>(manager, { type: "annotations.list" })).length,
    )
    .toBe(1);
...
test("production permission is optional, onboarding is reachable, and pages are untouched before grant", async () => {
  ...
  await expect(
    manager.getByRole("button", { name: "启用网页访问", exact: true }),
  ).toBeVisible();
  const page = await context.newPage();
  await page.goto("http://127.0.0.1:4173/article");
...
test("real text marking, reload and browser restart keep records", async () => {
  ...
  expect((await rpc<any[]>(manager, { type: "annotations.list" }))[0].id).toBe(
    before[0].id,
  );
  await cdp.detach();
  await context.close();
  ...
  await expect(reopened.locator(".web-ink-palette-toggle")).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  await expect
    .poll(() => reopened.evaluate(() => CSS.highlights.size))
    .toBeGreaterThan(0);
  expect((await rpc<any[]>(manager, { type: "annotations.list" }))[0].id).toBe(
    before[0].id,
  );
  await manager.screenshot({
    path: "test-results/library.png",
    fullPage: true,
  });
});
...
test("image drawing stores normalized points and follows responsive image size", async () => {
  ...
  await page.mouse.move(box!.x + 310, box!.y + 200, { steps: 6 });
  await page.mouse.up();
  await expect
    .poll(
      async () =>
  ...
  expect(record.shape.kind).toBe("rectangle");
  expect(record.shape.points[0].x).toBeGreaterThan(0);
  expect(record.shape.points[1].x).toBeLessThan(1);
  ...
  await page.locator("#diagram").evaluate((img) => {
...
test("changed source is unresolved, backup remains complete, and stale writes fail", async () => {
  ...
  await page.locator("#selection strong").evaluate((element) => {
    element.textContent = "The original source has been changed completely.";
  });
  const tabs = await manager.evaluate(() => chrome.tabs.query({}));
  const tab = tabs.find((t) => t.url === "http://127.0.0.1:4173/article")!;
  await expect
    .poll(
      async () =>
        (await rpc<any>(manager, { type: "page.state.get", tabId: tab.id }))
          .states[0]?.status,
    )
    .toBe("unresolved");
  const updated = await rpc<any>(manager, {
    type: "annotations.put",
    annotation: { ...record, note: "A note saved elsewhere" },
  ...
  await expect(
    rpc(manager, {
      type: "annotations.put",
      annotation: { ...record, note: "Stale draft" },
      expectedRevision: record.revision,
    }),
  ...
  expect(await rpc(manager, { type: "backup.preview", backup })).toMatchObject({
    identical: 1,
    conflicts: 0,
  });
  await expect(
    rpc(manager, {
      type: "backup.import",
      backup: { ...backup, schemaVersion: 999 },
      overwrite: true,
    }),
  ).rejects.toThrow();
  expect((await rpc<any[]>(manager, { type: "annotations.list" })).length).toBe(
    1,
  );
});
...
test("all four image tools persist and session undo/redo preserves the drawing", async () => {
  ...
  for (const [index, name] of ["方框", "椭圆", "箭头", "画笔"].entries()) {
    ...
    await page.mouse.move(box.x + 100 + index * 12, box.y + 100);
    await page.mouse.down();
    await page.mouse.move(box.x + 280 + index * 12, box.y + 180, { steps: 12 });
    await page.mouse.up();
    await expect
      .poll(
        async () =>
  ...
  expect(
    (await rpc<any[]>(manager, { type: "annotations.list" })).map(
      (a) => a.shape.kind,
    ),
  ).toEqual(["rectangle", "ellipse", "arrow", "pen"]);
  ...
  await page.getByRole("button", { name: "完成", exact: true }).click();
  await page.reload();
  await expect(page.locator("g[data-annotation-id]")).toHaveCount(4);
  await page.locator("#diagram").scrollIntoViewIfNeeded();
...
test("site pause/resume and manual rebind retain the original record", async () => {
  ...
  await rpc(manager, {
    type: "settings.put",
    settings: { ...settings, disabledOrigins: ["http://127.0.0.1:4173"] },
  });
  ...
  await expect
    .poll(() => page.evaluate(() => CSS.highlights.size))
    .toBeGreaterThan(0);
  await page.locator("#selection strong").evaluate((el) => {
    el.textContent = "A revised statement selected by the reader.";
  });
  const tab = (await manager.evaluate(() => chrome.tabs.query({}))).find(
    (t) => t.url === "http://127.0.0.1:4173/article",
  )!;
  ...
  await expect
    .poll(
      async () =>
        (await rpc<any[]>(manager, { type: "annotations.list" }))[0]?.revision,
    )
    .toBe(2);
  const [after] = await rpc<any[]>(manager, { type: "annotations.list" });
  expect(after.id).toBe(before.id);
  expect(after.target.exact).toContain("A revised statement");
});
...
test("library editing, conflict preservation, JSON download and import work through the UI", async () => {
  ...
  await manager.screenshot({
    path: "test-results/library-editing.png",
    fullPage: true,
  });
});
...
test("storage panel updates counts after create/delete and separates estimates from exact disk usage", async () => {
  ...
  await selectAndMark(page);
  await expect(panel).toContainText("1 条标注");
  await expect(panel).toContainText("1 个网页");
  ...
  await expect(panel).toContainText("0 条标注");
  await manager.screenshot({
    path: "test-results/storage-panel.png",
    fullPage: true,
  });
});
...
test("SPA URL changes and streaming updates do not trigger cross-page errors", async () => {
  ...
  const tab = (await manager.evaluate(() => chrome.tabs.query({}))).find((t) =>
    t.url?.includes("conversation=second"),
  )!;
  await expect
    .poll(
      async () =>
    ...
    .toBe("http://127.0.0.1:4173/article?conversation=second");
  await expect
    .poll(
      async () =>
        (await rpc<any>(manager, { type: "page.state.get", tabId: tab.id }))
  ...
  await expect(page.locator(".web-ink-palette-toggle")).toHaveAttribute(
    "aria-pressed",
    "false",
  );
  ...
  expect(
    records.filter((r) => r.pageUrl === "http://127.0.0.1:4173/article"),
  ).toHaveLength(1);
  expect(
    records.filter((r) => r.pageUrl.endsWith("conversation=second")),
  ).toHaveLength(1);
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: () =>
        chrome.runtime.sendMessage({
          ...
          pageUrl: "http://127.0.0.1:4173/article",
        }),
    });
  ...
  const queries = await manager.evaluate(async (tabId) => {
    const result = await chrome.scripting.executeScript({
      target: { tabId },
      func: async () => ({
        previous: await chrome.runtime.sendMessage({
          type: "annotations.query",
          query: { pageUrl: "http://127.0.0.1:4173/article", limit: 50 },
        }),
        current: await chrome.runtime.sendMessage({
          type: "annotations.query",
          query: { pageUrl: location.href, limit: 50 },
        }),
      }),
    });
    return result[0]?.result;
  }, tab.id!);
  if (!queries) throw Error("The isolated-world query did not return a result");
  expect(queries.previous).toMatchObject({ ok: false, code: "PAGE_CHANGED" });
  expect(queries.current.ok).toBe(true);
  expect(queries.current.data.items).toHaveLength(1);
  await page.evaluate(() =>
    history.replaceState({}, "", "/article?conversation=third"),
  );
  await expect
    .poll(
      async () =>
  ...
  await expect
    ...
    .toBe("http://127.0.0.1:4173/article");
  await expect
    .poll(
      async () =>
        (await rpc<any>(manager, { type: "page.state.get", tabId: tab.id }))
...
test("dismissed background errors stay quiet while failed saves retain retry controls", async () => {
  ...
  await page.evaluate(() =>
    document.querySelector("main")!.append(document.createTextNode(" change ")),
  );
...
test("palette is quiet by default and remembers the page switch without deleting notes", async () => {
  ...
  const select = () =>
    page.locator("#selection strong").evaluate((element) => {
      const range = document.createRange();
      range.selectNodeContents(element);
      const selection = window.getSelection()!;
      selection.removeAllRanges();
  ...
  await second.goto("http://127.0.0.1:4173/another-article");
  await expect(second.locator(".web-ink-palette-toggle")).toHaveAttribute(
    "aria-pressed",
    "false",
  );
});
...
test("metadata changes preserve text ranges and scrolling reuses image SVG nodes", async () => {
  ...
  await page.evaluate(() => {
    (window as any).__savedRange = [...CSS.highlights.values()][0]!
      .values()
      .next().value;
  });
  await rpc(manager, {
    type: "annotations.put",
    annotation: { ...text, note: "Metadata only" },
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          [...CSS.highlights.values()][0]?.values().next().value ===
          (window as any).__savedRange,
      ),
    )
    .toBe(true);
  const tab = (await manager.evaluate(() => chrome.tabs.query({}))).find(
  ...
  await page.getByRole("button", { name: "完成", exact: true }).click();
  const [drawing] = (
    await rpc<any[]>(manager, { type: "annotations.list" })
  ).filter((r) => r.kind === "image");
  const group = page.locator(`g[data-annotation-id="${drawing.id}"]`);
  await group.evaluate((element) => {
    (window as any).__savedGroup = element;
    (window as any).__savedShape = element.querySelector(":scope > rect");
  });
  ...
  await expect
    .poll(() =>
      group.evaluate(
        (element) =>
          element === (window as any).__savedGroup &&
          element.querySelector(":scope > rect") ===
            (window as any).__savedShape,
      ),
    )
    .toBe(true);
});
...
test("shared dark preferences and 320px layout remain usable across library and webpage", async () => {
  ...
  await expect(manager.locator(".ink-app")).toHaveAttribute(
    "data-theme",
    "dark",
  );
  await expect(page.locator("web-ink-ui")).toHaveAttribute(
    "data-web-ink-theme",
    "dark",
  );
  ...
  await expect
    ...
    .toBe(true);
  await manager.screenshot({
    path: "test-results/settings-dark-narrow.png",
    fullPage: true,
  });
  await manager.getByRole("button", { name: "资料库", exact: true }).click();
  ...
  await expect
    ...
    .toBe(true);
  await manager.screenshot({
    path: "test-results/library-dark-narrow.png",
    fullPage: true,
  });
});
...
test("click or reselect existing text to remove it, undo preserves notes, and reload does not resurrect it", async () => {
  ...
  const saved = await rpc<any>(manager, {
    type: "annotations.put",
  ...
  await expect(
    page.getByRole("button", { name: "取消标注", exact: true }),
  ).toBeVisible();
...
test("existing image marks can be removed directly after reload", async () => {
  ...
  await page.getByRole("button", { name: "完成", exact: true }).click();
  await page.reload();
  await expect(page.locator("g[data-annotation-id]")).toHaveCount(1);
  const toggle = page.locator(".web-ink-palette-toggle");
  await toggle.click();
...
test("overlapping highlights remove only the chosen record and failed deletion keeps the mark", async () => {
  ...
  const page = await article();
  await selectAndMark(page);
  const [original] = await rpc<any[]>(manager, { type: "annotations.list" });
  await rpc(manager, {
    type: "annotations.put",
    annotation: {
      ...original,
  ...
  await clickHighlightedText(page);
  await page.getByRole("button", { name: "取消标注 1", exact: true }).click();
  await expect
    .poll(
      async () =>
        (await rpc<any[]>(manager, { type: "annotations.list" })).length,
  ...
  await manager.evaluate(async (tabId) => {
    await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        chrome.runtime.sendMessage = ((m: any) =>
          m.type === "annotations.delete"
            ? Promise.resolve({
                ok: false,
                code: "INTERNAL",
                error: "Synthetic delete failure",
              })
            : send(m)) as typeof chrome.runtime.sendMessage;
      },
    });
  }, tab.id!);
  await clickHighlightedText(page);
  await page.getByRole("button", { name: "取消标注", exact: true }).click();
  await expect(page.locator(".toast.error")).toContainText("取消失败");
  expect((await rpc<any[]>(manager, { type: "annotations.list" })).length).toBe(
    1,
  );
...
test("library removes an existing annotation through inline confirmation without native dialogs", async () => {
  ...
  await expect(
    manager.getByRole("button", { name: "确认删除", exact: true }),
  ).toBeVisible();
  expect((await rpc<any[]>(manager, { type: "annotations.list" })).length).toBe(
    1,
  );
  await manager.getByRole("button", { name: "确认删除", exact: true }).click();
  await expect(manager.locator(".annotation-row")).toHaveCount(0);
  await expect.poll(() => page.evaluate(() => CSS.highlights.size)).toBe(0);
...
test("query paging and cancellation preserve complete substring results in the browser", async () => {
  const now = "2026-09-18T00:00:00.000Z";
  const records = Array.from({ length: 1000 }, (_, i) => ({
    id: `search-${String(i).padStart(4, "0")}`,
    pageUrl: `https://example.test/${i % 4}`,
    pageTitle: "Synthetic search",
    color: i % 2 ? "#facc15" : "#4ade80",
    note: i % 7 === 0 ? "Rare Needle" : "ordinary",
    tags: i % 3 === 0 ? ["paper"] : [],
    createdAt: now,
    updatedAt: now,
    revision: 1,
  ...
  let cursor: any;
  do {
    const result = await rpc<any>(manager, {
      type: "annotations.query",
      query: {
        text: "RARE needle",
        tag: "paper",
    ...
    found.push(...result.items.map((r: any) => r.id));
    cursor = result.nextCursor;
  } while (cursor);
  expect(found).toEqual(
    records
      .filter((r) => r.note === "Rare Needle" && r.tags.includes("paper"))
      .map((r) => r.id)
      .reverse(),
  );
  const result = await manager.evaluate(async () => {
    ...
    const pending = chrome.runtime.sendMessage({
      type: "annotations.query",
      query: { text: "unmatched", requestId },
    });
  ...
  await rpc(manager, {
    type: "annotations.query.cancel",
    requestId: "finished-query",
  });
  expect(
    (
      await rpc<any>(manager, {
        type: "annotations.query",
        query: { limit: 50 },
      })
  ).items,
