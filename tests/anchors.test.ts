...
describe("text anchors", () => {
  it("captures Chinese text with a normalized reading offset and resolves it after a distant sibling insertion", () => {
    document.body.innerHTML =
      '<main id="story"><p>这是足够长的前文，用来保持选中文本之前的上下文不变。这里还有更多内容。</p><p id="target">中文 <em>重点</em> 文本</p></main>';
    ...
    document
      ...
      ?.insertAdjacentHTML("afterend", "<aside>新插入的同级说明文字。</aside>");
  ...
  it("treats inline markup and br as readable whitespace", () => {
    document.body.innerHTML =
      '<main><p id="line">alpha <strong>beta</strong><br>gamma</p></main>';
  ...
  it("does not rebind an edited selection to a similar string", () => {
    document.body.innerHTML =
      '<main id="story"><p id="source">prefix selected suffix</p></main>';
  ...
  it("fails closed when exact text and its context are ambiguous inside the stable container", () => {
    document.body.innerHTML =
      '<main id="story"><p id="stable">A目标B A目标B</p></main>';
  ...
  it("uses the replacement root only after exact text and context still verify", () => {
    document.body.innerHTML =
      '<main id="story"><p id="stable">before chosen after</p></main>';
...
describe("image anchors", () => {
  it("uses source plus stable selector/context to distinguish duplicate image URLs", () => {
    document.body.innerHTML =
      '<main><figure id="first"><img id="one" src="/same.png" alt="one"><figcaption>first caption</figcaption></figure><figure id="second"><img id="two" src="/same.png" alt="two"><figcaption>second caption</figcaption></figure></main>';
